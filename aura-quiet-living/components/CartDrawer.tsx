    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <h2 className="text-xl font-serif text-slate-900">Your Tray ({items.length})</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-slate-500">Your tray is empty.</p>
            </div>
          ) : (
            items.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className="flex gap-4 animate-fade-in-up">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 uppercase tracking-widest mt-1">{item.category}</p>
                  </div>
                  <button 
                    onClick={() => onRemoveItem(idx)}
                    className="text-xs text-red-400 hover:text-red-600 self-start underline underline-offset-4 transition-colors"
                  >
                    Remove
                  </button>
                </div>
        ...
        <div className="p-6 border-t border-slate-200 bg-slate-50">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-xl font-serif text-slate-900">${total}</span>
          </div>
          <p className="text-xs text-slate-400 mb-6 text-center">Delivery to seat immediately after takeoff or upon request.</p>
          <button 
            onClick={onCheckout}
            disabled={items.length === 0}
            className="w-full py-4 bg-blue-900 text-white uppercase tracking-widest text-sm font-bold hover:bg-blue-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
          >
            Proceed to Payment
          </button>
        </div>
